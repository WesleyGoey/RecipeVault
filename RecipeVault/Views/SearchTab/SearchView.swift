                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if viewModel.isSearching {
                        HStack(spacing: 0) {
                            SearchPickerTab(
                                title: "TheMealDB",
                                isSelected: viewModel.selectedTab == .theMealDB
                            ) {
                                withAnimation(.easeInOut(duration: 0.2)) {
                                    viewModel.selectedTab = .theMealDB
                                }
                            }
                            SearchPickerTab(
                                title: "Collections",
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 16) {
                        if viewModel.mealDBRecipes.isEmpty {
                            ...
                            ForEach(viewModel.mealDBRecipes.prefix(5)) {
                                recipe in
                                NavigationLink(
    ...
    private var collectionsGrid: some View {
        VStack {
            if viewModel.isLoadingCollections {
                ...
                Text(
                    "When users create public collections, they will appear here."
                )
                ...
                LazyVGrid(columns: columns, spacing: 20) {
                    ForEach(viewModel.collections) { collection in
                        ...
                        // 1. Dapatkan nama asli pembuat koleksi (Atau gunakan "Chef" jika gagal)
                        let authorName =
                            viewModel.creatorNames[collection.userId] ?? "Chef"

                        // 2. Beri fallback image jika URL kosong di Firebase
                        let validImage =
                        ...
                        NavigationLink(
                            ...
                            DiscoverCardView(
                                ...
                                author:
                                    "@\(authorName.uppercased().replacingOccurrences(of: " ", with: "_"))",
                                recipeCount: 0,
                                imageUrl: validImage
                            )
                        }
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .clipShape(Capsule())
                .shadow(
                    color: isSelected ? .black.opacity(0.05) : .clear,
                    radius: 4,
                    x: 0,
                    y: 2
                )
